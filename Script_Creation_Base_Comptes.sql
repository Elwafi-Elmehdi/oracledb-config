...
insert into client
values (seq_client.nextval, 'Naciri','Ahmed', 'R�sidence Nassim N12', 'Casablanca');
...
CREATE OR REPLACE TRIGGER Agence_compte_2_trig
BEFORE DELETE OR UPDATE OF NO ON AGENCE
FOR EACH ROW
DECLARE
X NUMBER := 0;
BEGIN
SELECT COUNT(*) INTO X FROM COMPTE_2@DBL_ENSIAS_1 WHERE AGENCE_NO =: OLD.NO;
IF X <> 0 THEN
    RAISE_APPLICATION_ERROR(-20122,"OP INTERDITE : AGENCE AYANT DES COMPTE SUR COMPTE_2");
END IF;
END;
/

CREATE OR REPLACE TRIGGER compte_2_Agence_trig
BEFORE INSERT OR UPDATE OF AGENCE_NO ON compte_2
FOR EACH ROW
DECLARE
x NUMBER := 0;
BEGIN
SELECT COUNT(*) INTO x FROM AGENCE@DBL_ENSIAS2 WHERE NO = : NEW.AGENCE_NO;
IF x=0 THEN RAISE_APPLICATION_ERROR(-20123,"OP INTERDITE : AGENCE INCONNUE");
END IF;
END;
/
